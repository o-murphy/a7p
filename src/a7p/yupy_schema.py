...
_schema = mapping().shape({
    'profile': required(mapping().shape({
        ...
        'bullet_name': required(string().max(50), 'Bullet name is required'),
        'short_name_top': required(string().max(8), 'Short name top is required'),
        'short_name_bot': required(string().max(8), 'Short name bottom is required'),
        'caliber': required(string().max(50), 'Caliber is required'),
        'device_uuid': string().max(50),
        'user_note': string().max(1024),

        # zeroing
        'zero_x': required(number().ge(-200000).le(200000).integer(), 'Zero X is required'),
        'zero_y': required(number().ge(-200000).le(200000).integer(), 'Zero Y is required'),

        # lists
        'distances': required(array().of(number().ge(100).le(300000).integer()).min(1).max(200)),
        'switches': array().of(
            mapping().shape({
                'c_idx': required(number().ge(0).le(255).integer()),
                'distance_from': required(mixed().one_of(['INDEX', 'VALUE'])),
                'distance': required(number().ge(100).le(300000).integer()),
                'reticle_idx': required(number().ge(0).le(255).integer()),
                'zoom': required(number().ge(0).le(255).integer()),
            })
        ).min(4),
        ...
        'c_zero_air_temperature': required(number().ge(-100).le(100).integer()),
        'c_zero_air_pressure': required(number().ge(3000).le(15000).integer()),
        'c_zero_air_humidity': required(number().ge(0).le(100).integer()),
        'c_zero_w_pitch': required(number().ge(-90).le(90).integer()),
        'c_zero_p_temperature': required(number().ge(-100).le(100).integer()),
...
_coef_rows_std_schema = required(
    ...
    ).min(1).max(5).test(_coef_rows_mv_validator),
...
_coef_rows_custom_schema = required(
    ...
    ).min(1).max(200).test(_coef_rows_mv_validator),
