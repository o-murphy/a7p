...
| -------------------------- | --------------------------------------------- | :------------: | :-------------: | ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| unix                       | `x64`, `x86`                                  |       ✔️        |        ✔️        | 🔷 🔶             | the `natmod (x64)` / `natmod (x86)` jobs build and run the natmod path; `usermod` rows of the same names build and run the usermod path on the same runner, now via cibuildmp's own `v1.28.0-manylinux_2_28_x86_64`/`v1.28.0-manylinux_2_28_i686` Docker images — each is a genuinely native `x86_64`/`i686` container, so CI passes no `MICROPY_FORCE_32BIT=1` any more. **Building `ports/unix` locally, without cibuildmp/Docker, on a plain `x86_64` host still needs `MICROPY_FORCE_32BIT=1`** for `x86` (adds `-m32` to the host gcc; needs `gcc-multilib` + an i386 libffi) — that flag is a bare-host concern, not a CI one, and stays true regardless of what CI itself does. Both natmod/usermod are kept because the two modes fail differently — a usermod links against the port's own libc, a natmod against dynruntime |
| unix                       | `aarch64`                                     |       ❌        |        ✔️        | 🔶                | `usermod` job, `port: unix`, cibuildmp identifier `v1.28.0-manylinux_2_28_aarch64`, `runs_on: ubuntu-24.04-arm` — native build inside cibuildmp's own native-`arm64` Docker image, no cross-compiler                                                                                                                                                                                                |
| unix                       | `armhf`                                       |       ❌        |        ✔️        | 🔶                | dedicated `usermod-cross` job (`arch_label: armhf`), cibuildmp identifier `v1.28.0-manylinux_2_31_armv7l`, `runs_on: ubuntu-24.04-arm` — cross-built inside cibuildmp's own Docker image (`CROSS_COMPILE=arm-linux-gnueabihf-`, standalone/deplibs handled internally by cibuildmp's `build_unix()`), then **run on the arm64 runner's own CPU — no qemu, no binfmt handler** |
| unix                       | `mipsel`                                       |       ❌        |        ✔️        | 🔶                | same dedicated `usermod-cross` job (`arch_label: mipsel`), cibuildmp identifier `v1.28.0-manylinux_2_39_mipsel` — little-endian, deliberately not upstream's big-endian `mips-linux-gnu`: this module hardcodes `uctypes.LITTLE_ENDIAN` (a7p.py) over memory nanopb populates in the host's native byte order, so big-endian genuinely mismatches (confirmed by a real CI run: build+QEMU run succeeded, `test_a7p.py`'s zero_x/zero_y/sc_height assertion failed). Proven working on mipsel by ballistics-lab/micropython-bclibc's own CI |
| windows                    | `x86`                                         |       ❌        |        ✔️        | 🔶                | `usermod-windows-build` job, cibuildmp identifier `v1.28.0-win32`, `runs_on: ubuntu-latest` — cross-built inside cibuildmp's own Docker image (apt-installed `mingw-w64-i686-gcc`, no MSYS2 at build time at all); `usermod-windows-test` downloads the `.exe` and runs it on `windows-latest` (WOW64) |
| windows                    | `x64`                                         |       ❌        |        ✔️        | 🔶                | `usermod-windows-build` job, cibuildmp identifier `v1.28.0-win_amd64`, `runs_on: ubuntu-latest` — same Docker cross-build (apt `mingw-w64-x86_64-gcc`); `usermod-windows-test` runs it on `windows-latest`                                                                                                                                                   |
| windows                    | `arm64`                                       |       ❌        |        ✔️        | 🔶                | `usermod-windows-build` job, cibuildmp identifier `v1.28.0-win_arm64`, `runs_on: ubuntu-latest` — same Docker cross-build via `llvm-mingw` (`CFLAGS_EXTRA=-Wno-error` and friends resolved internally by cibuildmp's `build_windows()`); `usermod-windows-test` runs it on `windows-11-arm` |
| webassembly                | `wasm`                                        |       ❌        |        ✔️        | 🔶                | `usermod` job, `port: webassembly`, cibuildmp identifier `v1.28.0-wasm32` (`pyscript` variant), `runs_on: ubuntu-latest`, tests via `run_wasm_tests.mjs` — full suite (module now uses its own vendored `_a7p._md5()`, not `hashlib.md5`)                                                                                                                |
| rp2                        | `armv6m`, `armv7emsp`, `armv7emdp`, `rv32imc` |       ✔️        |        ✔️        | 🔷 🔶             | `natmod` job builds all 4 ARCHes; plus a `usermod-rp2040` job, cibuildmp identifier `v1.28.0-rp2-RPI_PICO`, that builds `BOARD=RPI_PICO` firmware inside cibuildmp's own Docker image and **runs** `test_a7p_core.py` on it under the rp2040py emulator. That execution is the point: natmod builds `armv6m` and never runs it, so this is the only RP2040 execution this project has |
| esp32                      | `xtensawin` (+ `rv32imc` for C3/C6)           |       ✔️        |        ✔️        | 🔷 🔶             | `natmod` job, `arch: xtensawin`; plus a `usermod-esp32` **build-only** job, cibuildmp identifier `v1.28.0-esp32-ESP32_GENERIC` — cibuildmp's own `esp32` driver resolves ESP-IDF v5.5.1 and runs the build inside its own `esp_idf_base` Docker image, no separate ESP-IDF-install step here any more. There is no esp32 emulator to run a firmware image on, so that job proves the module links into a real esp32 firmware and nothing more |
