...
/* Clamps every out-of-range numeric field a7p_validate() above checks (see
 * a7p_validate.c's a7p_clamp() for exactly which fields and why some
 * violations -- structural ones, not range ones -- are deliberately left
 * alone). Returns -1 if there's no profile (nothing to clamp), otherwise
 * the number of individual values changed (0 means everything was already
 * in range). Does not guarantee a7p_validate() succeeds afterwards --
 * clamping numeric ranges is not the same as fixing every structural rule
 * (coef_rows item count below its minimum, mv duplicates). */
int a7p_clamp(profedit_Payload *payload);
