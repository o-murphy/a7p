...
static mp_obj_t a7p_clamp_fn(mp_obj_t buf_obj) {
    mp_buffer_info_t buf_info;
    mp_get_buffer_raise(buf_obj, &buf_info, MP_BUFFER_WRITE);
    if (buf_info.len < A7P_PAYLOAD_SIZE) {
        mp_raise_ValueError(MP_ERROR_TEXT("a7p: buffer too small for Payload"));
    }
    int n = a7p_clamp((profedit_Payload *)buf_info.buf);
    return mp_obj_new_int(n);
}
static MP_DEFINE_CONST_FUN_OBJ_1(a7p_clamp_obj, a7p_clamp_fn);

/* One-shot MD5 digest of an in-memory buffer, backed by the vendored
 * Solar Designer implementation (md5.c/md5.h) rather than hashlib.md5 --
 * a7p.py's load()/dump() need this unconditionally for the .a7p file
 * format's checksum prefix, and firmware built with MICROPY_PY_SSL off
 * (common on flash-constrained targets) has no hashlib.md5 at all. See
 * README.md's "Firmware dependency: hashlib.md5" section. */
static mp_obj_t a7p_md5(mp_obj_t data_obj) {
    mp_buffer_info_t data_info;
    mp_get_buffer_raise(data_obj, &data_info, MP_BUFFER_READ);

    MD5_CTX ctx;
    MD5_Init(&ctx);
    MD5_Update(&ctx, data_info.buf, data_info.len);
    uint8_t digest[16];
