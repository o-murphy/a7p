#!/usr/bin/env python3
"""Regenerates micropython/src/a7p_layout.h and the generated block in a7p.py.
...
offsetof()/sizeof(), runs it, and emits:
  ...
  - src/a7p.py         the uctypes descriptor, rewritten in place between
                       the "# BEGIN GENERATED" / "# END GENERATED" markers
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        cmd = [
            ...
            *PB_DEFINES,
            ...
            SRC_DIR,
            probe_c,
            os.path.join(SRC_DIR, "profedit.pb.c"),
            "-o",
            probe_bin,
        ]
        subprocess.run(cmd, check=True)
        out = subprocess.run(
            [probe_bin], check=True, capture_output=True, text=True
        ).stdout
    sizes = {}
    fields = {}
...
def gen_python(sizes, fields):
    ...
    for name, (off, size) in fields["profedit_Profile"].items():
        ...
        if name == "switches":
            ...
            out.append(
                f'    "coef_rows": ({off} | uctypes.ARRAY, {count}, COEFROW_DESC),'
            )
        elif name == "distances":
            count = size // 4
            out.append(
                f'    "distances": ({off} | uctypes.ARRAY, {count} | uctypes.INT32),'
            )
...
BEGIN_MARKER = "# BEGIN GENERATED (tools/gen_layout.py) -- do not edit by hand"
END_MARKER = "# END GENERATED"


def splice_a7p_py(py_path, generated):
    ...
    if not pattern.search(content):
        sys.exit(
            f"error: no {BEGIN_MARKER!r} ... {END_MARKER!r} block found in {py_path}"
        )
    replacement = BEGIN_MARKER + "\n" + generated.rstrip("\n") + "\n" + END_MARKER
    content = pattern.sub(lambda _: replacement, content, count=1)
...
def main():
    ...
    py_path = os.path.join(SRC_DIR, "a7p.py")
    with open(header_path, "w") as f:
        f.write(gen_header(sizes, fields))
    splice_a7p_py(py_path, gen_python(sizes, fields))
    print(f"wrote {header_path}")
    print(f"updated generated block in {py_path}")
