...
class A7PFactory:
    ...
    def __new__(  # type: ignore[misc]  # intentional: A7PFactory(...) is called as a Payload-returning function, never instantiated
        ...
        def fmt_bottom():
            ...
            return (
                "{:.{}f}".format(bullet.weight, 0 if bullet.weight % 1 == 0 else 1)
                + "gr"
            )
        ...
        def drag_model():
            return [
                profedit_pb2.CoefRow(
                    bc_cd=round(point.coeff * 10000), mv=round(point.velocity * 10)
                )
        ...
        return profedit_pb2.Payload(
            profile=profedit_pb2.Profile(
                ...
                short_name_top=meta.short_name_top
                if meta.short_name_top
                else meta.name[:6],
                short_name_bot=meta.short_name_bot
                if meta.short_name_bot
                else fmt_bottom(),
